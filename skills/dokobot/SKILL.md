...
This Skill runs inside loli-plugin. Do not invoke Bash or raw `dokobot` CLI commands. Use the plugin tools instead:

- Search the web with `dokobot_search`.
- Read or continue reading a URL with `dokobot_read` (pass `sessionId` when supplied).
- Capture a page with `dokobot_screenshot`.
- Download page images with `dokobot_download_images`.
- Close a read session with `dokobot_close_session`.

The shell commands below are reference documentation only. Prefer local-mode tools and never expose browser content or credentials to unrelated users.
